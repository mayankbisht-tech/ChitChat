...
const ProfilePage = () => {
  ...
  const [bio,setBio]=useState<string>("I am using ChitChat to connect with my friends and family.");
      ...
      <div className='w-5/6 max-w-2xl backdrop-blur-2xl text-gray-300 border-2 border-gray-600 flex items-center justify-between ,max-sm:flex-col-reverse rounded-lg '>
        <form onSubmit={handleSubmit} className='flex flex-col gap-5 p-10 flex-1' action="">
          ...
          <label htmlFor="avatar" className='flex items-center gap-3 cursor-pointer'>
            <input onChange={(e)=>setSelectedImage(e.target.files ? e.target.files[0] : null)} type="file" id='avatar' accept='.png,.jpg,.jpeg' hidden />
          ...
          <textarea onChange={(e)=>setBio(e.target.value)} value={bio} placeholder="Your profile bio" id="bio" required className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-violet-500' rows={4}></textarea>
