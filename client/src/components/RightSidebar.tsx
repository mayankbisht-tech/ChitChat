...
const RightSidebar = ({selectedUser,setSelectedUser}: {selectedUser: boolean | any, setSelectedUser: React.Dispatch<React.SetStateAction<boolean | any>>}) => {
  return selectedUser && (
          ...
          <div className='mt-2 max-h-[200px] overflow-y-scroll grid grid-cols-2 gap-4 opacity-80'>
            {imagesDummyData.map((url,index) => (
              ...
              className="cursor-pointer rounded">
                <img key={index} src={url} alt={`media-${index}`} className="h-full rounded-md" />
